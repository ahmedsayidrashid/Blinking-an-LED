...
void main() {
    ...
    gpio_set_function(15, GPIO_FUNC_PWM); // set gpio 2 as the pwm output
    uint slice_num = pwm_gpio_to_slice_num(15); // get the pwm channel of this specific pin
    pwm_set_enabled(slice_num, 1); // enable the pwm at 
    int duty_factor = 10;
    int clock_divider = 12500;
    pwm_set_wrap(slice_num, clock_divider); // set the wrap point
